export default {
  async fetch(request, env) {
    ...
    if (!quizId) {
      ...
      originUrl.hostname = 'drama.theviralapps.com';
    ...
    try {
      const jsonUrl = `https://quiz-lp.pages.dev/quiz-drama/${quizId}.json`;
      ...
      if (data.quiz && data.quiz.content) {
        ...
        language = wpQuiz.settings?.language || detectLanguage(counterTemplate, firstQuestion, privacyText);
      } else {
        language = rawQuiz?.settings?.language || 'es';
      }
      ...
      quiz = {
        ...
        settings: {
          ...
          counter_min: rawQuiz?.settings?.counter_min || 10000,
          counter_max: rawQuiz?.settings?.counter_max || 20000,
          update_interval: rawQuiz?.settings?.update_interval || 10,
          language: language
        },
      ...
      const privacyUrlMap = {
        pt: 'https://drama.theviralapps.com/pt/politica-de-privacidade/',
        en: 'https://drama.theviralapps.com/privacy-policy/',
        es: 'https://drama.theviralapps.com/es/politica-de-privacidad/',
        tr: 'https://drama.theviralapps.com/tr/gizlilik-politikasi/',
        fr: 'https://drama.theviralapps.com/fr/politique-de-confidentialite/'
      };
...
<title>Drama - ${escapeHtml(quiz.title)}</title>
<link rel="icon" type="image/png" href="https://drama.theviralapps.com/wp-content/uploads/2025/04/drama-ico-2.png">
...
<script>
(function(){var q="${quizId}",a="https://estatisticas-six.vercel.app/api/track",c=false,v=false;function sid(){var s=sessionStorage.getItem("quiz_sid");if(!s){s="s_"+Date.now()+"_"+Math.random().toString(36).substr(2,9);sessionStorage.setItem("quiz_sid",s)}return s}function g(){try{var p=new URLSearchParams(window.location.search);return p.get("utm_campaign")||null}catch(e){return null}}function s(e){var u=g(),sid_val=sid();fetch(a,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({event:e,quizId:q,utm_campaign:u,session_id:sid_val})}).catch(function(){})}function setup(){var vk="qv_"+q;if(!v&&!sessionStorage.getItem(vk)){setTimeout(function(){v=true;sessionStorage.setItem(vk,"1");s("view")},1000)}function complete(){if(!c){c=true;s("complete")}}var okBtn=document.getElementById("okBtn");var playBtn=document.getElementById("playBtn");if(okBtn)okBtn.addEventListener("click",complete);if(playBtn)playBtn.addEventListener("click",complete)}if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",setup)}else{setup()}})();
</script>
</body>
</html>`;
      ...
      if (!yesRedirectUrl && answers.length > 0) {
        for (const answer of answers) {
          ...
          if (val === 'yes' || val === 'sim' || val === 'sí' || val === 'option_0') {
      ...
      const privacyUrl = quiz.assets.privacyPolicyUrl || privacyUrlMap[language] || privacyUrlMap.es;
...
<title>Drama - ${escapeHtml(quiz?.title || 'Quiz')}</title>
<link rel="icon" type="image/png" href="https://drama.theviralapps.com/wp-content/uploads/2025/04/drama-ico-2.png">
...
${quiz.assets.logoUrl ? `<div class="quiz-logo"><img src="${quiz.assets.logoUrl}" alt="Drama" width="180" height="60"></div>` : ''}
...
${quiz.settings.show_counter ? `var mn=${quiz.settings.counter_min},mx=${quiz.settings.counter_max},t="${currentTexts.watching}",loc="${currentLocale}";function up(){var v=Math.floor(Math.random()*(mx-mn+1))+mn,e=document.getElementById("c");if(e)e.textContent=v.toLocaleString(loc)+" "+t}setInterval(up,${quiz.settings.update_interval}*1e3);` : ''}
...
<script>
(function(){var q="${quizId}",a="https://estatisticas-six.vercel.app/api/track",c=false,v=false;function sid(){var s=sessionStorage.getItem("quiz_sid");if(!s){s="s_"+Date.now()+"_"+Math.random().toString(36).substr(2,9);sessionStorage.setItem("quiz_sid",s)}return s}function g(){try{var p=new URLSearchParams(window.location.search);return p.get("utm_campaign")||null}catch(e){return null}}function s(e){var u=g(),sid_val=sid();fetch(a,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({event:e,quizId:q,utm_campaign:u,session_id:sid_val})}).catch(function(){})}function setup(){var vk="qv_"+q;if(!v&&!sessionStorage.getItem(vk)){setTimeout(function(){v=true;sessionStorage.setItem(vk,"1");s("view")},1000)}function complete(){if(!c){c=true;s("complete")}}var btns=document.querySelectorAll(".quiz-button");btns.forEach(function(btn){btn.addEventListener("click",complete)});var il=document.getElementById("imglink");if(il){il.addEventListener("click",complete)}}if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",setup)}else{setup()}})();
</script>
</body>
</html>`;
